...
namespace Mageseller\ProductImport\Model\Reader;

use Exception;
use Mageseller\ProductImport\Api\Data\BundleProduct;
use Mageseller\ProductImport\Api\Data\BundleProductOption;
use Mageseller\ProductImport\Api\Data\BundleProductSelection;
use Mageseller\ProductImport\Api\Data\BundleProductStoreView;
use Mageseller\ProductImport\Api\Data\ConfigurableProduct;
use Mageseller\ProductImport\Api\Data\CustomOption;
use Mageseller\ProductImport\Api\Data\CustomOptionValue;
use Mageseller\ProductImport\Api\Data\DownloadableProduct;
use Mageseller\ProductImport\Api\Data\DownloadableProductStoreView;
use Mageseller\ProductImport\Api\Data\DownloadLink;
use Mageseller\ProductImport\Api\Data\DownloadSample;
use Mageseller\ProductImport\Api\Data\GroupedProduct;
use Mageseller\ProductImport\Api\Data\GroupedProductMember;
...
use Mageseller\ProductImport\Api\Data\SourceItem;
use Mageseller\ProductImport\Api\Data\TierPrice;
use Mageseller\ProductImport\Api\Data\VirtualProduct;
use Mageseller\ProductImport\Api\Data\Weee;
use Mageseller\ProductImport\Api\Importer;
        ...
        if ($scope === self::IMPORT) {
            ...
            if ($element === self::DOWNLOAD_LINK) {
                $this->downloadLink = new DownloadLink(
                    $attributes['file_or_url'],
                    $attributes['number_of_downloads'],
                    $attributes['is_shareable'],
                    $attributes['sample_file_or_url']
                );
            }
            ...
            if ($element === self::CUSTOM_OPTION_TEXTFIELD) {
                $this->customOptions[] = $this->customOption = CustomOption::createCustomOptionTextField(
                    $attributes['sku'],
                    $attributes['required'],
                    $attributes['max_characters']
                );
            } elseif ($element === self::CUSTOM_OPTION_TEXTAREA) {
                $this->customOptions[] = $this->customOption = CustomOption::createCustomOptionTextArea(
                    $attributes['sku'],
                    $attributes['required'],
                    $attributes['max_characters']
                );
            } elseif ($element === self::CUSTOM_OPTION_FILE) {
                $this->customOptions[] = $this->customOption = CustomOption::createCustomOptionFile(
                    $attributes['sku'],
                    $attributes['required'],
                    $attributes['file_extensions'],
...
*/
            ...
            if ($element === self::PRODUCT_SELECTION) {
                $this->productSelections[] = new BundleProductSelection(
                    $attributes['sku'],
                    $attributes['is_default'],
                    $attributes['price_type'],
                    $attributes['price_value'],
                    $attributes['quantity'],
                    $attributes['can_change_quantity']
                );
            }
        } elseif ($scope === self::DOWNLOAD_LINKS) {
            if ($element === self::DOWNLOAD_LINK) {
