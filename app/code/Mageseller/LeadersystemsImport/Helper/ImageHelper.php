...
class ImageHelper extends ProductHelper
{
    ...
    const INGRAMMICRO_IMPORTCONFIG_IS_ENABLE = 'leadersystems/importconfig/is_enable';
    ...
    public function __construct(
        ...
        parent::__construct($context, $leadersystemsimportLogger, $processResourceFactory, $importerFactory, $utilityHelper);
           ...
           (
            ...
            while (false !== ($row = $file->readCsv())) {
                try {
                    ++$i;
                    $this->start = microtime(true);
                    $sku =  (string) $row[$this->headers[self::STOCK_CODE]];
                    if (!in_array($sku, $allLeadersystemsSkus)) {
                        continue;
                    }
                    $flag = false;
                    $product = new SimpleProduct($sku);
                    $imageUrl = (string) $row[$this->headers['IMAGE']];
                    if ($imageUrl) {
