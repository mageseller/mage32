<?php
namespace Mageseller\Process\Model;
...
use Mageseller\Process\Model\Output\Factory as OutputFactory;
use Mageseller\Process\Model\Output\OutputInterface;
...
class Process extends AbstractModel
{
    ...
    {
        return !$this->isProcessing() && $this->getMagesellerStatus() == self::STATUS_PENDING;
    }
    ...
    {
        $fileSize = $this->getFileSize($isMageseller);
    ...
    {
        $this->addOutput('db');
        $this->output('Checking Mageseller report status...');
        ...
        try {
            ...
            $this->setMagesellerStatus(self::STATUS_PROCESSING);
            ...
            if ($synchroResult->getStatus() != 'COMPLETE') {
                $this->setMagesellerStatus(self::STATUS_PENDING);
            ...
            if ($synchroResult->getData('has_report')) {
                ...
                if ($filepath = $this->processHelper->saveFile($reportFile)) {
                    $fileSize = $this->processHelper->formatSize(filesize($filepath));
                    $this->setMagesellerFile($filepath);
    ...
    {
        $filePath = $isMageseller ? $this->getMagesellerFile() : $this->getFile();
    ...
    {
        $file = $isMageseller ? $this->getMagesellerFile() : $this->getFile();
    ...
    {
        if ($fileSize = $this->getFileSize($isMageseller)) {
