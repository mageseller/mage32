...
class ProductHelper extends AbstractHelper
{
    ...
    const INGRAMMICRO_CATEGORY_TABLE = "mageseller_ingrammicroimport_ingrammicrocategory";
    const PDF_FOLDER = 'devicesPdf';
    const SUPPLIER = 'ingrammicrodistribution';
    /**
     * /**
    ...
    /** @var MetaData */
    protected $metaData;
    private $existingIngrammicroCategoryIds;
    ...
    {
        try {
            $allIngrammicroSkus = $this->getAllIngrammicroSkus();
            $allSkus = [];
            $allCategoryNames = [];
            ...
            $disableSkus = array_diff($allIngrammicroSkus, $allSkus);

            $this->existingIngrammicroCategoryIds = $this->getExistingIngrammicroCategoryIds($allCategoryNames);
    ...
    private function getExistingIngrammicroCategoryIds($allCategoryNames)
    {
        if (empty($allCategoryNames)) {
            return [];
        }
        $ingrammicroCategoryTable = $this->db->getFullTableName(self::INGRAMMICRO_CATEGORY_TABLE);
        $categoryCollection = $this->categoryCollectionFactory->create();
        $categoryCollection->addAttributeToSelect('ingrammicro_category_ids', 'left');
        $select = $categoryCollection->getSelect();
        $select->reset(Select::COLUMNS)->columns('GROUP_CONCAT(`e`.`entity_id`)');
        $select->where("FIND_IN_SET(`{$ingrammicroCategoryTable}`.`ingrammicrocategory_id`, `at_ingrammicro_category_ids`.`value`)");
        return $this->db->fetchMap("
            SELECT `name`, ({$select}) as `category_ids`  
            FROM `{$ingrammicroCategoryTable}`
    ...
    private function getAllIngrammicroSkus()
    {
        $option = $this->loadOptionValues('supplier');
        $ingrammicroOptionId = $option['ingrammicrodistribution'] ?? "";
        if ($ingrammicroOptionId) {
            $productCollection = $this->_productCollectionFactory->create();
            $productCollection->addAttributeToFilter('supplier', ['eq' => $ingrammicroOptionId ], 'left');
