...
class UpdateAttributeAction {
    ...
    function _prepareValueForSave($value, AbstractAttribute $attribute)
    {
    ...
    public function updateCategoryAttributes($productIds, $attrData, $storeId = 0)
    {
        $this->updateAttributes($productIds, $attrData, $storeId, \Magento\Catalog\Model\Category::ENTITY);
    }
    public function updateProductAttributes($productIds, $attrData, $storeId = 0)
    {
        $this->updateAttributes($productIds, $attrData, $storeId);
    }
    public function updateCustomerAttributes($productIds, $attrData, $storeId = 0)
    {
        $this->updateAttributes($productIds, $attrData, $storeId, \Magento\Customer\Model\Customer::ENTITY);
    }
    public function updateCustomerAddressAttributes($productIds, $attrData, $storeId = 0)
    {
        $this->updateAttributes($productIds, $attrData, $storeId, 'customer_address');
    }
}
$updateAttributeAction = new UpdateAttributeAction(
...
/*$includeInMenuAttribute = $updateAttributeAction->getAttribute('include_in_menu','catalog_category');
echo "<pre>";
print_r(json_decode(json_encode($includeInMenuAttribute->getData()),true));
die;*/
$start_time=microtime(true);

$storeId = $updateAttributeAction->getStoreId();
$entityId = 1;
$value = 0;
$Ids = [1,2,3];
$attributes = ['include_in_menu' => 1];
$updateAttributeAction->updateCategoryAttributes($Ids,$attributes);
